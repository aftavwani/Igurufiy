...
import { uploadImageId } from "../../components/uploadImageId";
...
const InstaRegistration: React.FC = () => {
  const history = useHistory();
  const isIosDevice = isPlatform("ios");
  const iosDeviceClass = isIosDevice == true ? "ios-upload-file" : "";
  ...
  const { photos, takePhoto, livePhoto, setlivePhoto } = usePhotoGallery();
  const { photosId, takePhotoId, livePhotoId, setlivePhotoId } =
    uploadImageId();

  const [livePhotoTextName, setLivePhotoImageTextName] = useState<any>(null);
  const handleFileSelect = async (e: any) => {
    e.preventDefault();
    setlivePhoto(e.target.files[0]);
    setLivePhotoImageTextName(e.target.files[0]);
  };

  const [livePhotoIdTextName, setLivePhotoIdImageTextName] =
    useState<any>(null);
  const handlePhotoId = async (e: any) => {
    e.preventDefault();
    setlivePhotoId(e.target.files[0]);
    setLivePhotoIdImageTextName(e.target.files[0]);
  };
  ...
  const onSubscription = (e: any) => {
    setSubscriptionData(e.target.checked);
    if (e.target.checked === true) {
      setifChecked(true);
    }
  };
  ...
  const [ig_usernameErrorMsg, setig_usernameErrorMsg] = useState("");
  const [subscriptionCheckbox, setsubscriptionCheckbox] = useState("");
  const [agreeCheckbox, setagreeCheckbox] = useState("");
  const [checkedData, setcheckedData] = useState(false);

  useIonViewWillLeave(() => {
    setLivePhotoIdImageTextName(null);
    setLivePhotoImageTextName(null);
  });
  ...
  const submitBtn = (data: any) => {
    ...
    if (checkNetwok === true) {
      ...
      PushNotifications.addListener("registration", (token: Token) => {
        ...
        formData.append("ig_username", validator.trim(data.ig_username));
        ...
        formData.append("check", "Agree");
        formData.append("checkk", "1");
        formData.append("ig_username_photo", livePhoto);
        formData.append("photo_id", livePhotoId);
        ...
        if (subscriptionData == false) {
          setsubscriptionCheckbox("Checkbox will be required");
        } else {
          if (checkboxData == true && subscriptionData == true) {
            setstartLoading(true);

            const Url = process.env.API_SOME_KEY + "register-Instagram/";
            axios({
              method: "post",
              url: Url,
              data: formData,
  ...
  const goBack = () => {
    ...
    if (ig_usernameErrorMsg.length != 0) {
      setig_usernameErrorMsg("");
    }
    ...
    if (subscriptionCheckbox.length != 0) {
      setsubscriptionCheckbox("");
    }
    setcheckedData(false);
    setifChecked(false);
  };
  ...
  const [modalIfChecked, setmodalIfChecked] = useState(false);
  const modalCheckbox = (e: any) => {
    if (e.target.checked === true) {
      setmodalIfChecked(true);
    } else {
      setmodalIfChecked(false);
    }
  };

  const [txtColor, settxtColor] = useState("dark");
  const modalAcceptBtn = () => {
    if (modalIfChecked === false) {
      setmodalIfChecked(false);
      settxtColor("danger");
    } else {
      modal.current?.dismiss();
  ...
  return (
    <>
        ...
        <IonContent fullscreen className="main">
          ...
          <div className="content registration">
            <IonText className="ion-text-center">
              <h4 className="adasdsf">Instagram Guru Registration</h4>
            </IonText>
            ...
            >
              <IonLabel color="light">Instagram Username</IonLabel>
              <IonInput
              ...
              />
              <div className="already-used">{ig_usernameErrorMsg}</div>

              <IonLabel color="light">Full Name</IonLabel>
              <IonInput
                {...register("name", {
              ...
              <IonLabel className="label" color="light">
                Select Industry
              </IonLabel>
              <IonItem className="select">
                <IonSelect
                  {...register("industry", {
                    required: "Select Industry is a required",
                  })}
                  aria-label="Industry"
                  interface="popover"
                  className="indust"
              ...
              <IonLabel className="label upload-label" color="light">
                Capture or Upload Photo Id
              </IonLabel>
              <div className="upload-file">
                <div className={"sign-upload-file social-media-img " + iosDeviceClass}>
                  {/* <input
                    type="file"
                    name="photo_id"
                  ...
                  <label
                    htmlFor="file-upload-insta-id"
                    // className="custom-file-upload"
                  >
                    {livePhotoIdTextName != null
                      ? livePhotoIdTextName.name
                      : "No file choosen"}
                  </label>
                  <input
                    id="file-upload-insta-id"
                    type="file"
                    onChange={handlePhotoId}
                    accept="image/*"
                    name="photo_id"
              ...
              <IonLabel className="label upload-label" color="light">
                Capture or Upload a selfie holding your Instagram username on a
                paper
              </IonLabel>
              <div className="upload-file">
                <div className={"sign-upload-file social-media-img " + iosDeviceClass}>
                  {/* <input
                    ...
                    accept="image/*"
                    name="ig_username_photo"
                  /> */}

                  <label
                    htmlFor="file-upload-insta-selfie"
                    // className="custom-file-upload"
                  >
                    {livePhotoTextName != null
                      ? livePhotoTextName.name
                      : "No file choosen"}
                  </label>
                  <input
                    id="file-upload-insta-selfie"
                    type="file"
                    onChange={handleFileSelect}
                    accept="image/*"
                    name="ig_username_photo"
              ...
              <IonLabel color="light">DOB of birth as on photo ID</IonLabel>
              <IonInput
                {...register("dob", {
                  required: "DOB is a required",
                })}
                placeholder="DOB"
                className="custom"
                name="dob"
              ...
              <ErrorMessage
                ...
                as={<div className="error" />}
              />
              <div className="already-used">{emailErrorMsg}</div>
              ...
              <ErrorMessage
                ...
                as={<div className="error" />}
              />
              <div className="already-used">{passwordErrorMsg}</div>

              <div className="ion-color-light checked1">
                <IonCheckbox
                  className="remember-checkbox remember"
                  labelPlacement="end"
                  name="check"
                  onIonChange={onChechbox}
                  checked={checkedData}
                  value="Agree"
                  id="open-instagram-terms"
                >
                ...
                <IonCheckbox
                  className="remember-checkbox remember"
                  labelPlacement="end"
                  onIonChange={onSubscription}
                  checked={ifChecked}
                  id="open-instagram-modal"
                  name="checkk"
                >
                ...
                <div className="check-error">{subscriptionCheckbox}</div>
              </div>
              <IonButton
                id="present-alert"
                expand="block"
      ...
      <IonModal
        ...
        ref={modalTerms}
        trigger={checkboxData === false ? "open-instagram-terms" : ""}
      >
      ...
      {/* //____  SUBSCRIPTION CHECKBOX MODAL START ____// */}
      <IonModal
        id="example-modal"
        backdrop-dismiss={false}
        className="premium-modal"
        ref={modal}
        trigger="open-instagram-modal"
      >
        <IonContent>
          <IonToolbar className="modal-tittle-content">
          ...
          <div className="popup-data">
            <IonList>
              <IonItem className="list-item">
                ...
                <p>Full Access to your Guru Profile</p>
              </IonItem>
              <IonItem className="list-item">
                <IonAvatar slot="start">
                  <IonIcon
                ...
                <p>
                  Access to respond back to reviews from past clints to control
                  your narrative
                </p>
              </IonItem>
              <IonItem className="list-item">
                ...
                <p>Access to FLAGE your reviews for re-evaluation</p>
              </IonItem>
              <IonItem className="list-item">
                <IonAvatar slot="start">
                  <IonIcon
                ...
                <p>48 Hour Costomer Service Response Time from IGURUFY Team</p>
              </IonItem>
              <IonItem className="list-item">
                <IonAvatar slot="start">
                  <IonIcon
                ...
                <p>
                  Access to Post via your GURU IGURUFY wall for past clients to
                  see any news you want highlight & Much more to come!
                </p>
              </IonItem>
            </IonList>
          </div>
          <div className="premium-checkbox">
            <IonCheckbox
              checked={modalIfChecked}
              onIonChange={modalCheckbox}
              labelPlacement="end"
              className="accept-checkbox"
            >
              <IonText color={txtColor} style={{ fontSize: "12px" }}>
                Check if you want to register
              </IonText>
            </IonCheckbox>
            <IonButton className="accept-btn" onClick={modalAcceptBtn}>
              Accept
            </IonButton>
          </div>
        </IonContent>
      </IonModal>
      {/* //____  SUBSCRIPTION CHECKBOX MODAL END ____// */}
    </>
  );
};
export default InstaRegistration;
